...
func createDockerEnvironment(numInstances int) (*dockerTestSetup, error) {
	...
	allowlistPeerID := p2pkey.GetID()
	setup.allowlistPeerID = allowlistPeerID
	...
	initEnv := []string{
		...
		"LOG_LEVEL=debug",
		"UNLIMITED_START=1",
		fmt.Sprintf("ALLOWLIST_PEERS=%s", allowlistPeerID),
	}
...
func startLocalManagerWithError(setup *dockerTestSetup, key *p2p.P2PKey, peerListChan chan peer.IDSlice, extDB p2psrv.ExternalDB) (func() error, []*p2p.P2PClient, error) {
	...
	if _, err := p2pMgr.NewGossipSub(context.Background(), "doltswarmdemo-gossip-v1"); err != nil {
		stopper()
		return nil, nil, fmt.Errorf("failed to init gossip for local manager: %w", err)
	}
...
func startLateJoiner(t *testing.T, setup *dockerTestSetup) *lateJoinerInfo {
	...
	env := []string{
		...
		"LOG_LEVEL=debug",
		"UNLIMITED_START=1",
		fmt.Sprintf("ALLOWLIST_PEERS=%s", setup.allowlistPeerID),
