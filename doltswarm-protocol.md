...
- **Multi-peer partition-heal regression checks in spec:** Beyond the pairwise sync convergence checks, the Quint model now includes a 3-peer partition-heal regression (`inv_three_peer_partition_heal_reconcilable`) plus bounded repeated-round full-mesh healing checks to catch cases where independently hardened histories either fail to remain merge-reconcilable or fail to collapse to one shared finalized tip under a deterministic fair sync schedule. In the current 3-peer model this is expressed as a two-phase bounded progress argument: frontier rank strictly decreases while more than one incomparable tip remains, and once the frontier rank reaches zero one more full-mesh round converges finalized state.
