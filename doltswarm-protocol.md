...
Design a peer-to-peer synchronization protocol for Dolt databases that allows multiple peers to concurrently read and write SQL data, automatically merge non-conflicting changes, surface conflicts to users for resolution, and converge on a shared finalized history through monotone append-only reconciliation — even after partitions cause different peers to harden different finalized lineages — all without a central coordinator, with minimal inter-peer communication, and with no consensus protocol.

More concretely, eventual consistency in doltswarm is not just "the same data eventually appears everywhere." The protocol also aims for eventual convergence of the finalized Dolt history itself without rollback or history rewriting: once connectivity is restored and anti-entropy continues, repeated deterministic sync rounds should monotonically reduce the frontier of incomparable finalized tips until peers share one common finalized tip, while preserving earlier finalized merges as ancestors.
...
That leads to a more precise eventual-consistency target for the history layer: partition healing does not need to collapse all divergence in one step, but it must be monotone and append-only. Different reconnection orders may create different intermediate merge commits, yet later sync rounds must still be able to join those intermediates into one shared finalized lineage without discarding any already-finalized history.
...
| `parked_conflicts` | `Set[EventID]` | Authoritative set of parked events. This includes both later-in-total-order heads that conflicted during tentative reconciliation and any immediately finalization-eligible head whose replay against the current finalized accumulator would already conflict or whose replay base is not yet deterministically reconstructable. Parked events are excluded from LOCAL_WRITE parents and finalization. During closure lag (some heads not mergeable yet), unresolved parked head IDs are retained conservatively until closure completes and reconciliation recomputes against a complete mergeable set. |
...
**No voting, no consensus, no coordinator.** There is no agreement mechanism in the protocol. Convergence follows from set union (the Merkle clock is a G-Set) and deterministic merge steps. For finalized history with more than two incomparable tips, convergence is by repeated deterministic `SYNC_FINALIZED` rounds that monotonically merge the remaining incomparable frontier; it is not based on one-shot associativity of nested pairwise merges.
...
`AnchoredFold(head_event_ids, start_prolly_root_hash) -> (root, conflict_set)` is the shared merge primitive used by `RECONCILE` and by `ADVANCE_STABILITY`'s replay-base reconstruction helper: sort head event IDs by HLC, start `acc = start_prolly_root_hash`, then fold `MergeRoots(acc, head.prolly_root_hash, head.anchor_prolly_root_hash)` so each head event is replayed against the finalized anchor it was originally written on.
...
6. Compute `replay_unsafe_ready = { h ∈ mergeable_head_event_ids | h ∉ SubsumedEvents(clock) ∧ h.parent_event_ids ⊆ finalized_events.keys() ∧ (ReplayBase(h) is undefined ∨ MergeRoots(finalized_prolly_root_hash, h.prolly_root_hash, ReplayBase(h)) conflicts) }`, where `ReplayBase(h)` uses the same rule as `ADVANCE_STABILITY`: root events replay against their recorded anchor; otherwise replay against the deterministic parent-basis root reconstructed from finalized parents, excluding `resolved_parked_event_id` if present.
7. `projection_ready = mergeable_head_event_ids \ replay_unsafe_ready`.
8. Sort `projection_ready` by HLC total order (deterministic).
9. Apply `AnchoredFold(projection_ready, finalized_prolly_root_hash)` to compute `(merged_root, parked_ready)`:
...
10. `retained_unready_parked = parked_conflicts ∩ unready_head_event_ids`.
11. `parked_conflicts ← parked_ready ∪ replay_unsafe_ready ∪ retained_unready_parked`.
12. `tentative_prolly_root_hash ← merged_root`.
13. `projection_basis ← projection_ready \ parked_ready`. This is the exact event-id basis embodied in `tentative_prolly_root_hash`.
14. **`head_event_ids` are unchanged.** Head event IDs are always derived from `computeHeadEventIDs(clock)` — since reconcile does not add events to the clock, the head-event-id set cannot change. Parked head event IDs remain as persistent DAG forks until `RESOLVE_CONFLICT`. Retained finalized clock bodies may still appear in `head_event_ids` structurally, but they do not re-enter tentative merge/parking once finalized.
...
3. Normal `LOCAL_WRITE` flow from step 4 (same `EventID` construction, with `resolved_parked_event_id = event_id` and `resolved_finalized_conflict_id = null`). `RECONCILE` in the `LOCAL_WRITE` flow recomputes `parked_conflicts` from scratch — the resolved event is no longer a head (the resolution event parents from it), so it leaves `parked_conflicts` deterministically. No manual removal is needed. Other previously parked heads are not manually preserved or cleared either: they remain parked only if the post-resolution deterministic recomputation still parks them; otherwise they may legitimately move into `projection_basis`. The explicitly resolved parked parent is a causal carry-forward parent, not part of the resolution event's replay basis during later finalization.
...
2. Define `ParkedBranchEvents(clock, parked_conflicts)` as each unresolved parked head plus its retained tentative ancestors. Then `newly_stable = { e ∈ clock | e.event_id ∉ finalized_events.keys() ∧ e.event_id ∉ ParkedBranchEvents(clock, parked_conflicts) ∧ e is PARENTS_READY ∧ e is CHUNKS_READY ∧ ∀ q ∈ active_peers: CoveredBy(peer_seen_frontier[q], e) }`. Unresolved parked branches cannot be finalized until resolved, and non-ready events (missing ancestors or chunks) cannot be finalized.
3. `parent_finalized_stable = { e ∈ newly_stable | e.parent_event_ids ⊆ finalized_events.keys() }`. This is the causal frontier of stable events whose stable ancestors are already compiled into finalized history.
4. Finalization eligibility is stricter than causal stability alone. For each `e ∈ parent_finalized_stable`:
  - if `e.event_id ∈ SubsumedEvents(clock)`, keep it eligible for bookkeeping-only finalization;
  - otherwise reconstruct `ReplayBase(e)` using the same replay-basis rule as normal replay finalization below, and require `MergeRoots(finalized_prolly_root_hash, e.prolly_root_hash, ReplayBase(e))` to be conflict-free.
  - Define `eligible_stable = { e ∈ parent_finalized_stable | e.event_id ∈ SubsumedEvents(clock) ∨ ReplaySafe(e) }`.
5. If `eligible_stable = ∅`, stop. Otherwise choose `next_event ∈ eligible_stable` with minimum HLC total order (deterministic).
6. Determine whether `next_event` is subsumed: `is_subsumed = next_event.event_id ∈ SubsumedEvents(clock)`.
7. **If `is_subsumed`:** bookkeeping-only finalization — the resolution event already carries this event's content forward, so no `MergeRoots` replay is needed.
   ...
   - Repeat from step 2.
8. **If not `is_subsumed`:** normal replay finalization.
9. Snapshot previous finalized commit/root once for this step: `prev_finalized_commit_id = finalized_commit_id`, `prev_finalized_prolly_root_hash = finalized_prolly_root_hash`.
10. Reconstruct the deterministic replay base for `next_event`.
  - If `next_event.parent_event_ids = ∅`, then `replay_base = next_event.anchor_prolly_root_hash`.
  - Otherwise let `replay_parents = next_event.parent_event_ids`, except that if `next_event.resolved_parked_event_id = x`, then `x` is excluded from `replay_parents` because it is a causal carry-forward parent, not part of the resolution event's replay basis.
  - Rebuild the prior parent-basis root as `replay_base = AnchoredFold(replay_parents, next_event.anchor_prolly_root_hash).root`. This reconstruction MUST be conflict-free and deterministic.
11. Compute `merged = MergeRoots(prev_finalized_prolly_root_hash, next_event.prolly_root_hash, replay_base)`.
  - **Normative requirement:** this merge MUST be conflict-free for the chosen replay event. A non-subsumed event enters `eligible_stable` only if its reconstructed replay base is defined and this replay is already conflict-free against the current finalized accumulator. Encountering a conflict here is therefore a protocol violation.
12. Bookkeeping finalization for exactly this event:
   - `finalized_events[next_event.event_id] ← next_event.prolly_root_hash`
   - if `next_event.resolved_finalized_conflict_id` is present, union that singleton id into durable `resolved_finalized_conflict_ids`
13. Determine the real finalized commit parents for this checkpoint:
...
14. Materialize one deterministic **checkpoint** finalized commit in Layer 2 from `FinalizedCommitPayload = (merged, parent_finalized_commit_ids_sorted, CheckpointFinalize)`. The resulting commit hash is the new `finalized_commit_id`.
15. Refresh tentative materialization, if any. `finalized_prolly_root_hash ← merged`.
16. Update derived finalized lineage metadata to reflect the materialized checkpoint commit:
...
17. Remove any id now in `resolved_finalized_conflict_ids` from `finalized_conflicts`.
18. Repeat from step 2, because finalizing `next_event` can make additional stable events become eligible.
...
3. Apply branch:
   ...
   6. Merge finalized bookkeeping:
      ...
      - `finalized_checkpoint_events ← merged_finalized_checkpoint_events`
      - `resolved_finalized_conflict_ids ← merged_resolved_ids`
   7. Define the deterministic **merge** finalized commit payload `FinalizedCommitPayload = (finalized_prolly_root_hash, sort({our_finalized_commit_id, their_finalized_commit_id}), LineageMerge)`. The payload's deterministic identity is the new `finalized_commit_id`. This is one binary finalized-frontier reduction step, not a claim that all incomparable finalized tips in the healed swarm have already been joined.
   ...
   11. If later finalized-metadata exchange reveals another incomparable finalized tip (or merge tip), repeat `SYNC_FINALIZED`. The protocol may therefore materialize merge-of-merge commits during partition healing. Previously materialized merge commits remain in finalized history as ancestors; they are not flattened away or rewritten.
4. **Retained finalized event bodies must stay ready or be compacted:** After metadata exchange and branch application, if some `event_id ∈ clock.keys()` is now also in `finalized_events.keys()`, the peer MUST ensure that retained finalized event body remains both parent-closed in the retained `clock` subgraph and chunk-ready locally (fetch missing chunks if needed). In particular, equal-tip metadata repair in `NOOP` may reveal retained finalized event bodies that were already present in `clock` but not yet locally ready; implementations MUST hydrate whatever retained finalized closure is missing and mark those retained finalized bodies ready before proceeding. If either condition cannot be maintained, that finalized event body MUST be compacted out of `clock` immediately, even if some non-finalized retained event still points at it. In that case the tentative side remains provisional until later anti-entropy restores the missing closure. A retained finalized clock event must not remain finalized-but-unready or finalized-but-orphaned locally. Any such local `clock` mutation MUST also refresh `head_event_ids` and `peer_seen_frontier[self]` before later finalization/reconcile logic relies on them. Note: this applies to **all branches**, including NOOP. Equal-tip metadata repair can expand `finalized_events` via merged checkpoint descriptors, causing previously non-finalized retained events to become finalized and eligible for compaction. When compaction changes `clock` and `head_event_ids`, the tentative projection becomes stale.
5. **Recompute tentative projection after all branches:** After steps 3-4 are complete, the peer MUST recompute `(tentative_prolly_root_hash, parked_conflicts, projection_basis)` via `RECONCILE` from the post-sync `clock`, `head_event_ids`, `chunks_ready`, and `finalized_prolly_root_hash`. This applies to **all branches** (ADOPT, NOOP, and MERGE), not only when `finalized_commit_id` or `finalized_prolly_root_hash` changed. Equal-tip metadata repair (NOOP) can change the retained clock structure via step 4 compaction, which invalidates the pre-sync tentative projection even though the finalized tip is unchanged. For ADOPT and MERGE, `RECONCILE` additionally re-anchors tentative events: each event is replayed against its recorded `anchor_prolly_root_hash` in `AnchoredFold` over the new finalized accumulator. This is CPU-only for already-mergeable head event IDs. If any head event ID is missing ancestors or root/anchor chunks, the projection remains provisional; implementations MUST NOT silently clear unresolved parked head IDs in this state, and MUST recompute on closure completion (ancestor pull completion, chunk completion, or subsequent `RECEIVE_EVENT`/heartbeat-triggered reconcile).
...
The Quint suite treats bootstrap/join as out-of-model for transition semantics but now exercises these postconditions with seeded states, including compacted checkpoint bootstrap, retained finalized-body bootstrap, and equal-tip finalized-metadata repair after join.
...
**INV3a — Finalized Identity Separation:** Finalized-history identity depends on the exact deterministic finalized payload, not on `prolly_root_hash` alone. Two finalized commits may legitimately carry the same `prolly_root_hash` yet remain distinct `finalized_commit_id` values if their finalized parent sets or finalized-commit kinds differ.

**INV3b — Checkpoint Descriptor Consistency:** If a finalized event body is retained in `clock` and also appears in a reachable checkpoint descriptor, the retained event's `prolly_root_hash` must equal the descriptor's recorded root for that `event_id`. Checkpoint descriptors are synchronized finalized metadata, not advisory local cache.
...
- **INV7a — No Unhandled Tentative Conflict:** `tentative_prolly_root_hash` is never a conflict sentinel. Every conflict detected by `MergeRoots` during reconciliation, and every immediately eligible head that is already replay-unsafe against the current finalized accumulator, is captured in `parked_conflicts`. No conflict result propagates into the working state.
- **INV7b — No Unhandled Finalized Conflict:** `finalized_prolly_root_hash` is never a conflict sentinel. `RECONCILE` must keep immediately replay-unsafe eligible heads parked, and `ADVANCE_STABILITY` must treat replay safety as part of non-subsumed eligibility: once unresolved parked branches and subsumed branches have been filtered out and the event's replay base has been reconstructed from its finalized parent basis (excluding any carried-forward parked parent on a resolution event), only conflict-free replay candidates may harden. `SYNC_FINALIZED` divergence merges handle conflicts by using the ours-side root (deterministic). No conflict result propagates into the finalized state.
...
- **INV7ha — Finalized Conflict Hardening:** An event-carried `resolved_finalized_conflict_id` is only a claim until that event hardens into finalized state or the durable resolved-id set arrives through `SYNC_FINALIZED`. Once hardened, the id must enter `resolved_finalized_conflict_ids` and be absent from `finalized_conflicts` thereafter.
...
**INV10a — Action-Boundary Finalized-State Discipline:** `RECEIVE_EVENT`, `GC_CLOCK`, and `EVICT_PEER` do not directly mutate authoritative finalized history or finalized-layer conflict metadata. `ADVANCE_STABILITY` may extend finalized history, harden durable finalized-conflict resolutions, and shrink `finalized_conflicts`, but it must not create a fresh finalized conflict id. Once unresolved parked branches and subsumed branches are filtered out, causal stability alone is not enough for replay finalization: replay of a chosen non-subsumed eligible stable event against the current finalized accumulator and its reconstructed replay base must already be conflict-free by construction.
...
The formal Quint specification lives in `specs/`. `specs/model/doltswarm_model.qnt` is the source of truth for the **core state machine**: event write, receive, reconcile, heartbeat, finalize, finalized-event compaction (`do_gc`), **tentative conflict resolution** (resolve_conflict), **finalized conflict resolution** (resolve_finalized_conflict), **network partitions** (connectivity model, peer eviction), and **partition recovery** (sync_finalized). `specs/doltswarm_verify.qnt` keeps the bounded whole-model regression suite over that model. The detailed proof layout, canonical Apalache proof artifacts, and typical commands are documented in `specs/README.md`. Partition conflicts from divergence merges in `SYNC_FINALIZED` are handled inline using the ours-side root (no blocking), while conflict metadata is persisted in `finalized_conflicts` until the corresponding conflict id reaches durable finalized-state resolution (`resolved_finalized_conflict_ids`). The spec models chunk availability with an explicit `chunks_ready` abstraction (`do_chunks_ready`). The swarm-wide admission predicate is modeled abstractly as `validEvent(e)`; concrete transport/auth envelopes remain outside the core model. The spec models eviction and reconnection via heartbeat re-adding to `active_peers`. Bootstrap/join is treated as an out-of-model transport/bootstrap contract: the model does not simulate state transfer as a core transition, but it does include seeded bootstrapped-state regressions for compacted checkpoints, retained finalized bodies, and equal-tip finalized-metadata repair. `init` remains the degenerate already-bootstrapped empty case.
...
- **Finalization ordering:** `ADVANCE_STABILITY` (§2.4) is a deterministic single-step compiler repeated to fixpoint: compute `newly_stable` from active-peer seen-frontier coverage, exclude unresolved parked branches and any immediately replay-unsafe eligible heads that `RECONCILE` is required to keep parked, restrict to parent-finalized stable events, then further restrict non-subsumed candidates to those whose replay base is defined and whose reconstructed replay against the current `finalized_prolly_root_hash` is already conflict-free. Causal stability alone is therefore not sufficient for replay finalization. Choose the minimum eligible event by HLC total order. If the event is subsumed (`SubsumedEvents(clock)`), finalize it as bookkeeping-only (no replay, no new checkpoint); otherwise reconstruct the event's replay base from its finalized parent basis (root events use the recorded anchor directly, while `RESOLVE_CONFLICT` excludes the explicitly resolved parked parent from that basis) and merge it against the current `finalized_prolly_root_hash`. Then materialize one deterministic checkpoint commit and repeat. The Quint spec models the same semantics via parked-branch filtering plus a subsumed/replay-safe split in `finalizeResult`.
- **Finalized-commit lineage metadata:** The protocol treats `finalized_commit_id` as the authoritative finalized-history identity and `finalized_commit_parents` / `finalized_checkpoint_events` as derived metadata extracted from already-materialized finalized commits, while `finalized_prolly_root_hash` remains content payload. `do_finalize` computes a new deterministic `finalized_commit_id` only after the exact parent list for the checkpoint commit is known, unions the newly hardened event into that commit's checkpoint descriptor set, `do_sync_finalized` computes one deterministic binary merge step only after the exact two current finalized parents are known, and lineage-LCA (`lineageLcaFinalizedCommit`) runs over the derived parent links. This keeps the finalized Dolt DAG identical across peers while allowing equal-tip metadata repair. Nested partition recovery correctness (INV3) comes from repeated binary merge-of-merges rounds over the remaining incomparable finalized frontier, not from associativity of composing pairwise merges in one expression.
...
- **Subsumption:** The protocol defines `SubsumedEvents(clock)` as the ancestor-closure of every event ID referenced by any clock event's `resolved_parked_event_id`, and separately defines unresolved parked-branch exclusion over each parked head's retained tentative ancestry. The spec models both via `subsumedEvents(st.clock)` and `parkedBranchEvents(st)`. `ADVANCE_STABILITY` / `do_finalize` first excludes unresolved parked branches from replay eligibility, then treats subsumed events as bookkeeping-only finalization (no `MergeRoots`, no new checkpoint commit).
...
- **Finalized commit identity abstraction in spec:** The protocol defines `FinalizedCommitID = hash(CanonicalEncode(FinalizedCommitPayload))`. The Quint model does not compute that hash numerically. Instead, it represents `FinalizedCommitID` as `(prolly_root_hash, canonical_token_list, kind)`, where the token list is built purely from the exact sorted parent finalized-commit ids. This is a model-only realization of the same injective, order-independent identity contract, chosen to avoid overflow-sensitive arithmetic encodings and mutable allocation order.
- **SYNC_FINALIZED regression checks in spec:** The Quint model includes branch-specific contract invariants (`inv_sync_contract_adopt`, `inv_sync_contract_noop`, `inv_sync_contract_merge`, `inv_sync_unresolved_conflicts_monotone`, `inv_sync_conflict_recorded`, `inv_sync_conflict_id_symmetric`) and pairwise convergence/frontier-reduction checks to catch lineage-mode and conflict-propagation regressions early.
- **Finalized-conflict hardening checks in spec:** The Quint model checks finalized-conflict structural integrity (`inv_finalized_conflict_visibility`), authenticity against merge semantics (`inv_finalized_conflict_authenticity`), explicit resolve-action contract behavior (`inv_resolve_finalized_conflict_contract`: emit a finalized-conflict resolution claim without directly mutating finalized history), end-to-end hardening plus later finalized-sync preservation (`inv_resolved_finalized_conflict_hardens_and_syncs`), and anti-resurrection (`inv_finalized_conflict_no_resurrection`).
- **Lineage/LCA regression checks in spec:** The Quint model additionally checks finalized-lineage integrity (`inv_lineage_well_formed`, `inv_lineage_closure`), LCA soundness (`inv_lineage_lca_sound`), sync metadata-union correctness (`inv_sync_metadata_union`, `inv_sync_lineage_union_monotone`), merge-parent edge recording (`inv_sync_merge_parent_edges`), pairwise frontier reduction (`inv_sync_merge_frontier_reduction`), bounded merge-of-merges reconcilability (`inv_sync_nested_merge_reconcilable`), one-step component-frontier bounded growth plus descendant preservation (`inv_heal_frontier_growth_bounded`), frontier reduction when the synced pair are actual frontier tips (`inv_frontier_merge_strictly_decreases`), one-step absorption once a connected component has already converged on one finalized state (`inv_component_converged_frontier_absorbing`), and bounded full-mesh rank/convergence checks (`inv_fullmesh_frontier_rank_strict_progress`, `inv_fullmesh_unique_frontier_round_converges`, `inv_fullmesh_heal_bounded_converges`, `inv_converged_finalized_state_absorbing`, `inv_fullmesh_heal_converged_absorbing`).
- **Finalized commit identity regression checks in spec:** Because the model now derives finalized commit ids canonically from finalized payload structure, it checks tip/state consistency (`inv_finalized_commit_id_matches_state`), finalized-event reconstruction from reachable checkpoint descriptors (`inv_finalized_events_derived_from_checkpoints`), parent-sensitive finalized identity (`inv_same_root_different_parents_distinct_finalized_commit_ids`), and the fact that same-root events need not collapse event history (`inv_same_prolly_root_does_not_collapse_event_history`).
- **Tentative/finalized boundary checks in spec:** The Quint model now checks that tentative-layer and resource-management actions do not silently mutate finalized history: `LOCAL_WRITE` preserves finalized state (`inv_write_preserves_finalized_state`), `RECONCILE` preserves finalized state (`inv_reconcile_preserves_finalized_state`), `RECEIVE_EVENT` preserves finalized state (`inv_receive_preserves_finalized_state`), `GC_CLOCK` preserves finalized state (`inv_gc_preserves_finalized_state`), and `EVICT_PEER` preserves finalized state (`inv_evict_preserves_finalized_state`). The detailed proof organization for these checks is documented in `specs/README.md`. The model also checks that immediately replay-unsafe eligible heads stay parked (`inv_replay_unsafe_eligible_heads_are_parked`), that normal `ADVANCE_STABILITY` adds at most the single lineage edge back to the previous `finalized_commit_id` (`inv_finalize_linear_extension_only`), never creates fresh finalized conflicts (`inv_finalize_does_not_create_finalized_conflicts`), reconstructs one deterministic replay base for each non-subsumed eligible event (`inv_finalization_replay_base_is_defined`), and only exposes replay-eligible finalization candidates once unresolved parked branches and subsumed branches have been filtered correctly and replay bases are reconstructed correctly (`inv_finalization_eligible_events_are_conflict_free`). This is the executable version of the protocol rule "merge roots continuously, merge commits only when finalized lineages meet."
- **Tentative conflict-resolution checks in spec:** The Quint model checks the explicit `RESOLVE_CONFLICT` contract (`inv_resolve_conflict_contract`): the emitted event is anchored to the current `finalized_prolly_root_hash`, parents from `projection_basis ∪ {resolved_parked_event}`, removes that parked head from the head/parked set after recomputation, and does not directly mutate finalized state. During later finalization, the resolved parked parent remains a causal carry-forward link but is excluded from replay-base reconstruction.
...
- **Multi-peer partition-heal regression checks in spec:** Beyond the pairwise sync convergence checks, the Quint model now includes a 3-peer partition-heal regression (`inv_three_peer_partition_heal_reconcilable`) plus bounded repeated-round healing checks under more than one fair schedule (`inv_fullmesh_heal_bounded_converges`, `inv_alt_scheduled_heal_round_frontier_nonincreasing`, `inv_alt_scheduled_heal_bounded_converges`) to catch cases where independently hardened histories either fail to remain merge-reconcilable or fail to collapse to one shared finalized tip under reasonable deterministic schedule variations. In the current 3-peer model this is still a bounded progress argument, not a full temporal fairness proof for arbitrary schedules.
- **Checkpoint-descriptor consistency checks in spec:** Because checkpoint descriptors are synchronized finalized metadata, the Quint model also checks retained-body consistency directly (`inv_checkpoint_descriptor_root_consistency`): if a retained finalized event body is still materialized in `clock`, its `prolly_root_hash` must agree with the descriptor payload for that `event_id`.
...
2. **Finalized-state synchronization:** Heartbeats carry the finalized commit/content pair plus a finalized-metadata digest. When a peer receives a heartbeat with a different `finalized_commit_id` or a different finalized-metadata digest, it triggers `SYNC_FINALIZED`. The empty finalized commit id is treated as ordinary lineage, so empty peers recover by the same ADOPT/NOOP/MERGE machinery. Each divergence-reduction step uses a single three-way `MergeRoots(ours, theirs, common_ancestor)` with lineage-LCA common ancestor recovery. If healing reveals more than two incomparable finalized tips, later heartbeats may trigger additional merge-of-merge `SYNC_FINALIZED` steps until one finalized frontier tip remains. Equal-tip metadata repair stays in `NOOP` and preserves `dolt log`.

**Fork-and-join topology:** The finalized Dolt commit DAG is no longer strictly linear after partition recovery. It has a fork at the point of partition and one or more later joins. Both sides' finalized commits are preserved as history — `dolt log` shows the full picture: pre-split linear chain, parallel branches during partition, and later merge-of-merge joins if healing discovers several independently hardened frontier tips.
...
**Nested partitions:** If a partition splits further (e.g., all three peers isolated), the same logic applies recursively. Reconnection uses lineage-based `SYNC_FINALIZED` modes; merge cases produce deterministic binary merge commits in the Dolt DAG. The topology becomes more complex but remains well-defined and deterministic. Different first heal pairings may create different intermediate merge commits, but those intermediates remain ordinary finalized history and are later joined by further deterministic `SYNC_FINALIZED` rounds rather than being flattened or rewritten.

**Determinism:** All peers independently compute the same next merge result for the same pair of finalized tips. The "ours" vs "theirs" assignment is deterministic (lower finalized-commit id = "ours"), and the merge commit metadata is deterministic (derived from the two finalized states). For more than two incomparable finalized tips, convergence comes from repeated deterministic merge-of-merges rounds, not from one-shot associativity of nested pairwise merges. INV3 holds after convergence.
...
- Are excluded from `LOCAL_WRITE` parents and finalization. More precisely, an unresolved parked head and its retained tentative ancestors are blocked from finalized replay until that parked branch is either resolved or compacted away. Reconciliation recomputes parking when closure is ready; during closure lag unresolved parked heads are retained conservatively.
...
When a parked event is unresolved, the problem is not limited to that one head. Earlier tentative ancestors on the same parked branch may also become unsafe to replay once finalized history has advanced past their original anchor. A related case also occurs after a competing branch hardens: a head can become the only remaining tentative head yet still be unsafe to replay into the new finalized accumulator. Those retained tentative ancestors, and any such immediately replay-unsafe eligible head, must therefore stay parked until resolution.

**Fix: parked-branch blocking plus explicit subsumption.** `RESOLVE_CONFLICT` emits a resolution event with `resolved_parked_event_id = event_id`, marking the parked branch as carried forward. The protocol uses two derived sets:

- `ParkedBranchEvents(clock, parked_conflicts)`: each unresolved parked head plus its retained tentative ancestors. These are excluded from finalization while the parked branch remains unresolved.
- `SubsumedEvents(clock)`: the ancestor-closure of every event ID referenced by any resolution event's `resolved_parked_event_id` field. These events are finalized as bookkeeping-only entries because their content is already carried forward by the resolution event.

`ADVANCE_STABILITY` checks these conditions before replay:

- **Parked-branch events** are not eligible for replay finalization while the parked head remains unresolved.
- **Replay-unsafe eligible heads** remain parked even if they become the only tentative head after another branch hardens.
- **Replay-unsafe stable ancestors** are also deferred from replay finalization. Even after an event becomes causally stable and parent-finalized, it is not replay-eligible until its reconstructed replay base is defined and replaying it into the current finalized accumulator is conflict-free.
- **Subsumed events** are finalized as bookkeeping-only entries: added to `finalized_events` and attached to the current checkpoint descriptor, but no `MergeRoots` replay, no change to `finalized_prolly_root_hash`, and no new checkpoint commit.
- **Other non-subsumed events** follow the normal replay path. A `MergeRoots` conflict for the chosen replay event is a protocol violation.

This is safe because:

1. Events on an unresolved parked branch, immediately replay-unsafe eligible heads, and any other non-subsumed stable event whose reconstructed replay is still unsafe are excluded from replay finalization until they become safe or are later subsumed by resolution.
2. Once the resolution event arrives and `RECONCILE` recomputes, the resolved parked head leaves `parked_conflicts` and `SubsumedEvents(clock)` covers that resolved branch's retained tentative ancestry.
3. Finalization follows parent order: an event is only finalized after its causal parents are already in `finalized_events`.
4. Subsumption is derived from the clock, not from separate synchronized state. The resolution event propagates through the event DAG and is available on every peer before bookkeeping-only finalization of the subsumed branch can proceed.
...
Events are processed individually via `MergeRoots`. There is no batching into wall-time epochs. Tentative reconciliation replays head events against their recorded `anchor_prolly_root_hash`, but it also parks any immediately eligible head whose finalized replay base is already undefined or conflicting against the current finalized accumulator. Finalized replay then reconstructs the chosen event's replay base from its finalized parent basis (with `RESOLVE_CONFLICT`'s carried parked parent excluded from that basis) and merges against the current finalized accumulator. This is simpler than epoch-based batching and avoids epoch boundary edge cases.

### 7.3.1 Merges are silent local state

Merges (`RECONCILE`) are deterministic local computations derived from the current tentative-projection inputs: retained `clock`, `chunks_ready`, `finalized_prolly_root_hash`, and the locally derived `finalized_events` catalog. Retained finalized event bodies may still exist structurally in `clock`, but they remain below the tentative boundary and are excluded from tentative heads. Peers with the same tentative-projection inputs therefore compute the same `tentative_prolly_root_hash` and `projection_basis` whenever at least one tentative head is mergeable; under closure lag they conservatively retain unresolved parked tentative heads until closure completes, and once parents are finalized they also keep replay-unsafe eligible heads parked instead of letting them leak back into the live projection. After a finalized re-anchor followed by closure catch-up, recomputation uses that same rule: fold only `projection_ready = mergeable_head_event_ids \ replay_unsafe_ready`, not every ready head indiscriminately. They produce no events, no network traffic, and no DAG entries.
...
**Decision: accept the fork.** Both sides' finalized histories are treated as valid. Each `SYNC_FINALIZED` merge step uses a single `MergeRoots` call to join one currently incomparable pair of finalized tips into a new shared finalized base. The Dolt commit DAG gets a fork-and-join topology — and if healing reveals several independently hardened tips, later sync rounds may join earlier merge commits as well. No finalization is rolled back. No data is lost.
