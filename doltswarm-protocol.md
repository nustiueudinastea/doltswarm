...
- **Lineage/LCA regression checks in spec:** The Quint model additionally checks finalized-lineage integrity (`inv_lineage_well_formed`, `inv_lineage_closure`), LCA soundness (`inv_lineage_lca_sound`), sync metadata-union correctness (`inv_sync_metadata_union`), merge-parent edge recording (`inv_sync_merge_parent_edges`), pairwise frontier reduction (`inv_sync_merge_frontier_reduction`), and bounded merge-of-merges reconcilability (`inv_sync_nested_merge_reconcilable`).
